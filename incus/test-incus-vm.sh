...
#===============================================================================
# Incus VM Test Script
# 
# Tests: VM accessibility, Incus installation, storage, networking,
#        container operations via HTTP, profiles, and security
#
# Usage: ./test-incus-vm.sh [vm-name]
# Default VM name: incus-vm
#===============================================================================
...
test_incus_installation() {
    ...
    # Check incus client
    print_test "Incus client available"
    if vm_run "incus --help" && grep -qi "incus" "$TMPFILE"; then
        pass
    else
        fail "Incus client not found"
    fi
    ...
    # Check incus socket
    print_test "Incus socket accessible"
    if vm_run "test -S /var/lib/incus/unix.socket && echo ok" && [ "$(output)" = "ok" ]; then
        pass
    else
        fail "Incus socket not found"
    fi
    
    # Check Zabbly repository configured
    print_test "Zabbly repository configured"
    if vm_run "test -f /etc/apt/sources.list.d/zabbly-incus-lts.sources && echo ok" && [ "$(output)" = "ok" ]; then
        pass
    else
        warn "Zabbly repository file not found"
    fi
}
...
#-------------------------------------------------------------------------------
test_incus_storage() {
    print_header "Incus Storage Tests"
    
    # Check default storage pool exists
    print_test "Default storage pool exists"
    if vm_run "incus storage list --format=csv" && grep -q "default" "$TMPFILE"; then
        pass
        vm_run "incus storage show default | grep 'driver:'"
    ...
    if vm_run "incus storage info default" && grep -qi "used by" "$TMPFILE"; then
        pass
    else
...
#-------------------------------------------------------------------------------
# Test: Incus Networking
#-------------------------------------------------------------------------------
test_incus_networking() {
    print_header "Incus Networking Tests"
    
    # Check incusbr0 bridge exists
    print_test "Incus bridge (incusbr0) exists"
    if vm_run "incus network list --format=csv" && grep -q "incusbr0" "$TMPFILE"; then
        pass
    else
        fail "incusbr0 network not found"
    fi
    
    # Check bridge configuration
    print_test "Bridge has IPv4 address"
    if vm_run "incus network get incusbr0 ipv4.address"; then
    ...
    if vm_run "incus network get incusbr0 ipv4.nat" && [ "$(output | head -1)" = "true" ]; then
        pass
    else
    ...
    if vm_run "ip link show incusbr0" && grep -q "incusbr0" "$TMPFILE"; then
        pass
    else
        fail "incusbr0 interface not found in system"
    fi
}
...
#-------------------------------------------------------------------------------
test_incus_profiles() {
    print_header "Incus Profile Tests"
    
    # Check default profile exists
    print_test "Default profile exists"
    if vm_run "incus profile list --format=csv" && grep -q "default" "$TMPFILE"; then
        pass
    else
    ...
    vm_run "incus profile show default"
...
#-------------------------------------------------------------------------------
test_incus_images() {
    print_header "Incus Image Tests"
    
    # Check remote image servers
    print_test "Images remote configured"
    vm_run "incus remote list --format=csv"
    ...
    if vm_run "incus image list --format=csv" && grep -qi "ubuntu" "$TMPFILE"; then
        pass
...
test_web_container() {
    ...
    if vm_run "incus list web-test --format=csv"; then
        local line=$(output)
    ...
    if vm_run "incus config device show web-test" && grep -q "type: proxy" "$TMPFILE"; then
        pass
...
test_container_operations() {
    ...
    if vm_run "incus exec web-test -- cat /etc/os-release" && grep -qi "ubuntu\|PRETTY" "$TMPFILE"; then
        pass
    else
    ...
    if vm_run "incus exec web-test -- ping -c 1 -W 3 8.8.8.8" && grep -q "bytes from" "$TMPFILE"; then
        pass
    else
    ...
    if vm_run "incus file push /tmp/test-push-${test_id}.txt web-test/tmp/"; then
        pass
    else
    ...
    if vm_run "incus file pull web-test/tmp/test-push-${test_id}.txt /tmp/test-pull-${test_id}.txt && cat /tmp/test-pull-${test_id}.txt" && grep -q "test-content-${test_id}" "$TMPFILE"; then
        pass
    else
...
test_snapshot_operations() {
    ...
    if multipass exec "$VM_NAME" -- incus snapshot create web-test "$snap_name" 2>/dev/null; then
        pass
    else
    ...
    if vm_run "incus info web-test" && grep -q "${snap_name}" "$TMPFILE"; then
        pass
    else
    ...
    if multipass exec "$VM_NAME" -- incus snapshot delete web-test "${snap_name}" 2>/dev/null; then
        pass
    else
...
test_api_access() {
    ...
    if vm_run "incus config get core.https_address"; then
...
check_common_issues() {
    ...
    # Check Incus service health
    print_test "Incus service healthy"
    if vm_run "systemctl is-active incus" && [ "$(output | head -1)" = "active" ]; then
        pass
    else
        warn "Incus service may have issues"
...
print_summary() {
    ...
    if [ $TESTS_FAILED -eq 0 ]; then
        ...
        echo ""
        echo -e "  ${GREEN}Incus VM is healthy and ready to use.${NC}"
        echo ""
...
main() {
    echo ""
    echo -e "${BLUE}╔════════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${BLUE}║                   INCUS VM TEST SUITE                          ║${NC}"
