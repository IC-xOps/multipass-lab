...
test_podman_operations() {
    ...
    if multipass exec "$VM_NAME" -- incus exec podman-host -- podman run --rm docker.io/alpine echo "hello" > "$TMPFILE" 2>&1; then
    ...
    if multipass exec "$VM_NAME" -- incus exec podman-host -- podman run --rm docker.io/alpine ping -c 1 -W 3 8.8.8.8 > "$TMPFILE" 2>&1; then
...
test_rootless_podman() {
    ...
    if multipass exec "$VM_NAME" -- incus exec podman-host -- su - ubuntu -c 'podman run --rm docker.io/alpine echo rootless' > "$TMPFILE" 2>&1; then
