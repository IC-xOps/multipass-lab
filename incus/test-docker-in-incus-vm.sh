...
test_docker_operations() {
    ...
    if multipass exec "$VM_NAME" -- incus exec docker-host -- docker run --rm alpine echo "hello" > "$TMPFILE" 2>&1; then
    ...
    if vm_run "incus exec docker-host -- docker images alpine --format '{{.Repository}}'"; then
    ...
    if multipass exec "$VM_NAME" -- incus exec docker-host -- docker run --rm alpine ping -c 1 -W 3 8.8.8.8 > "$TMPFILE" 2>&1; then
