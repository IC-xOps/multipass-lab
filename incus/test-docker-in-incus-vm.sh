...
#===============================================================================
# Docker-in-Incus VM Test Script
# 
# Tests: VM accessibility, Incus installation, Docker profile, Docker container,
#        Docker functionality, and HTTP access via proxy device
#
# Usage: ./test-docker-in-incus-vm.sh [vm-name]
# Default VM name: docker-in-incus-vm
#===============================================================================

VM_NAME="${1:-docker-in-incus-vm}"
...
TMPFILE="/tmp/docker_incus_test_$$.txt"
...
test_docker_profile() {
    ...
    if vm_run "incus profile list --format=csv" && grep -q "docker" "$TMPFILE"; then
        pass
    else
    ...
    if vm_run "incus profile show docker" && grep -q 'security.nesting.*true' "$TMPFILE"; then
        pass
    else
...
test_docker_host_container() {
    ...
    if vm_run "incus config show docker-host" && grep -q "docker" "$TMPFILE"; then
        pass
    else
    ...
    if vm_run "incus list docker-host --format=csv"; then
        local line=$(output)
    ...
    if vm_run "incus config device show docker-host" && grep -q "type: proxy" "$TMPFILE"; then
        pass
...
test_docker_inside_container() {
    ...
    if vm_run "incus exec docker-host -- docker --version"; then
        local ver=$(output | head -1)
    ...
    if vm_run "incus exec docker-host -- systemctl is-active docker" && [ "$(output)" = "active" ]; then
        pass
    else
    ...
    if vm_run "incus exec docker-host -- docker info" && grep -q "Server Version" "$TMPFILE"; then
        pass
    else
    ...
    if vm_run "incus exec docker-host -- systemctl is-active containerd" && [ "$(output)" = "active" ]; then
        pass
    else
...
test_docker_containers() {
    ...
    if vm_run "incus exec docker-host -- docker ps --format '{{.Names}}'"; then
    ...
    if vm_run "incus exec docker-host -- docker ps --filter name=web --format '{{.Status}}'"; then
    ...
    if vm_run "incus exec docker-host -- docker images --format '{{.Repository}}:{{.Tag}}'"; then
...
test_docker_operations() {
    ...
    if multipass exec "$VM_NAME" -- incus exec docker-host -- docker run --rm alpine:3.21 echo "hello" > "$TMPFILE" 2>&1; then
    ...
    # Test docker pull (check if alpine exists)
    print_test "Docker pull works"
    if vm_run "incus exec docker-host -- docker images alpine:3.21 --format '{{.Repository}}'"; then
    ...
    if multipass exec "$VM_NAME" -- incus exec docker-host -- docker run --rm alpine:3.21 ping -c 1 -W 3 8.8.8.8 > "$TMPFILE" 2>&1; then
...
test_docker_compose() {
    ...
    if vm_run "incus exec docker-host -- docker compose version"; then
        local ver=$(output | head -1)
...
test_security_config() {
    ...
    if vm_run "incus exec docker-host -- cat /proc/self/cgroup" && grep -q "docker\|0::" "$TMPFILE"; then
        pass
    else
    ...
    if vm_run "incus config get docker-host raw.lxc"; then
        pass
    else
        pass
    fi
    ...
    if vm_run "incus exec docker-host -- stat -fc %T /sys/fs/cgroup/"; then
        local cgroup=$(output)
...
test_permissions() {
    ...
    if vm_run "incus exec docker-host -- stat -c '%a' /var/run/docker.sock"; then
    ...
    if vm_run "incus exec docker-host -- groups ubuntu"; then
    ...
    if vm_run "incus exec docker-host -- touch /tmp/test-write && incus exec docker-host -- rm /tmp/test-write && echo ok"; then
        if [ "$(output)" = "ok" ]; then
            pass
        else
...
test_resources() {
    ...
    if vm_run "incus exec docker-host -- free -m | awk '/^Mem:/{print \$2}'"; then
        local mem=$(output | tr -d '[:space:]')
...
print_summary() {
    ...
    if [ $TESTS_FAILED -eq 0 ]; then
        ...
        echo ""
        echo -e "  ${GREEN}Docker-in-Incus is working correctly.${NC}"
        echo ""
        echo -e "  ${BLUE}Web container accessible at: http://${VM_IP}:8080${NC}"
        echo ""
        echo -e "  ${BLUE}Access Docker:${NC}"
        echo -e "    multipass exec $VM_NAME -- incus exec docker-host -- docker ps"
    else
...
main() {
    echo ""
    echo -e "${BLUE}╔════════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${BLUE}║            DOCKER-IN-INCUS VM TEST SUITE                       ║${NC}"
