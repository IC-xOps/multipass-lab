...
#===============================================================================
# Podman-in-LXD VM Test Script
# 
# Tests: VM accessibility, LXD installation, Podman profile, Podman container,
#        Podman functionality, rootless support, and HTTP access via proxy
#
# Usage: ./test-podman-in-lxd-vm.sh [vm-name]
# Default VM name: podman-in-lxd-vm
#===============================================================================

VM_NAME="${1:-podman-in-lxd-vm}"
...
TMPFILE="/tmp/podman_lxd_test_$$.txt"
...
test_podman_profile() {
    ...
    if vm_run "lxc profile list --format=csv" && grep -q "podman" "$TMPFILE"; then
        pass
    else
    ...
    if vm_run "lxc profile show podman" && grep -q 'security.nesting.*true' "$TMPFILE"; then
        pass
    else
...
test_podman_host_container() {
    ...
    if vm_run "lxc config show podman-host" && grep -q "podman" "$TMPFILE"; then
        pass
    else
    ...
    if vm_run "lxc list podman-host --format=csv"; then
        local line=$(output)
    ...
    if vm_run "lxc config device show podman-host" && grep -q "type: proxy" "$TMPFILE"; then
        pass
...
test_podman_inside_container() {
    ...
    if vm_run "lxc exec podman-host -- podman --version"; then
        local ver=$(output | head -1)
    ...
    if vm_run "lxc exec podman-host -- podman info --format '{{.Host.OCIRuntime.Name}}'"; then
    ...
    if vm_run "lxc exec podman-host -- which podman-compose"; then
        pass
    else
    ...
    if vm_run "lxc exec podman-host -- which slirp4netns"; then
        pass
    else
...
test_podman_containers() {
    ...
    if vm_run "lxc exec podman-host -- podman ps --format '{{.Names}}'"; then
    ...
    if vm_run "lxc exec podman-host -- podman ps --filter name=web --format '{{.Status}}'"; then
    ...
    if vm_run "lxc exec podman-host -- podman images --format '{{.Repository}}:{{.Tag}}'"; then
...
test_podman_operations() {
    ...
    # Test podman images (check alpine exists from previous run)
    print_test "Podman pull works"
    if vm_run "lxc exec podman-host -- podman images --format '{{.Repository}}'"; then
...
test_rootless_podman() {
    ...
    if vm_run "lxc exec podman-host -- grep ubuntu /etc/subuid"; then
    ...
    if vm_run "lxc exec podman-host -- grep ubuntu /etc/subgid"; then
        if grep -q "ubuntu" "$TMPFILE"; then
            pass
        else
...
test_docker_compatibility() {
    ...
    if vm_run "lxc exec podman-host -- podman --help" && grep -q "Manage pods, containers and images" "$TMPFILE"; then
        pass
    else
        warn "Podman CLI may differ from Docker"
    fi
    
    # Check podman socket (for docker-compose compatibility)
    print_test "Podman socket available"
    if vm_run "lxc exec podman-host -- ls /run/podman/podman.sock 2>/dev/null || echo 'no socket'"; then
...
test_security_config() {
    ...
    if vm_run "lxc exec podman-host -- cat /proc/self/cgroup" && grep -q "0::" "$TMPFILE"; then
        pass
    else
    ...
    if vm_run "lxc exec podman-host -- stat -fc %T /sys/fs/cgroup/"; then
        local cgroup=$(output)
    ...
    if vm_run "lxc exec podman-host -- which fuse-overlayfs"; then
        pass
...
test_permissions() {
    ...
    if vm_run "lxc exec podman-host -- test -S /run/podman/podman.sock && echo exists || echo missing"; then
    ...
    if vm_run "lxc exec podman-host -- cat /etc/subuid"; then
    ...
    if vm_run "lxc exec podman-host -- touch /tmp/test-write && lxc exec podman-host -- rm /tmp/test-write && echo ok"; then
        if [ "$(output)" = "ok" ]; then
            pass
        else
    ...
    if vm_run "lxc exec podman-host -- stat -c '%U:%G' /home/ubuntu"; then
...
test_resources() {
    ...
    if vm_run "lxc exec podman-host -- free -m | awk '/^Mem:/{print \$2}'"; then
        local mem=$(output | tr -d '[:space:]')
...
print_summary() {
    ...
    if [ $TESTS_FAILED -eq 0 ]; then
        ...
        echo ""
        echo -e "  ${GREEN}Podman-in-LXD is working correctly.${NC}"
        echo ""
        echo -e "  ${BLUE}Web container accessible at: http://${VM_IP}:8080${NC}"
        echo ""
        echo -e "  ${BLUE}Access Podman:${NC}"
        echo -e "    multipass exec $VM_NAME -- lxc exec podman-host -- podman ps"
        echo ""
        echo -e "  ${BLUE}Rootless Podman:${NC}"
        echo -e "    multipass exec $VM_NAME -- lxc exec podman-host -- sudo -u ubuntu podman ps"
    else
...
main() {
    echo ""
    echo -e "${BLUE}╔════════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${BLUE}║             PODMAN-IN-LXD VM TEST SUITE                        ║${NC}"
